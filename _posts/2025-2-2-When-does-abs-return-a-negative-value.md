...
As said, if we want to negate the 32 bit number -2 147 483 648, it appears we can't: +2 147 483 648 does not exist as a signed 32 bit int. But if you do it anyway, what happens? Well the obvious place would be next to +2 147 483 647 (0x7FFF FFFF). A sensible candiate for +2 147 483 648 would be one higher. That would be 0x8000 0000. However, incrementing 0x7FFF FFFF *overflows* into the region reserved for negative numbers. So 0x8000 0000 is actually a negative number, and as it happens it is the SAME VALUE as is used for -2 147 483 648. 
...
## Appendix: a counting argument
Initially, I did not like the overflow explanation. It sounds ... incidental? At some point I had the following explanation, which I think is sound, but too long, so I add it as an appendix. Feel free to try if you like it better.
For negative numbers, abs() negates the number. A property of _negation_ is "double negate is a no-op": negating a number twice produces that same number. With that property in mind, let's review all our numbers. 0 negates to itself.The numbers +1 to +2 147 483 647 that negate to -1 to -2 147 483 647 **and vice versa**. So they also have that property. 
